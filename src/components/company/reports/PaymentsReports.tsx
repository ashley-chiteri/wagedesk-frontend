// src/components/company/payroll/reports/PaymentsReports.tsx
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Landmark, Smartphone, Wallet, Eye, Download, Loader2 } from "lucide-react";
...
const PAYMENT_REPORTS = [
  { type: "bank-payment", label: "Bank Payment File", icon: Landmark, format: "CSV" },
  { type: "mpesa-payment", label: "M-Pesa Payment File", icon: Smartphone, format: "CSV" },
  { type: "cash-payment", label: "Cash Payment Sheet", icon: Wallet, format: "PDF" },
];
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Payment processing files for various payment methods
          </p>
        </div>
        <Badge 
          variant="outline" 
      ...
      {!selectedRun && (
        <div className="bg-gray-50 border border-gray-200 p-6 text-center">
          <p className="text-sm text-gray-500">
            Select a payroll run to enable payment files
          </p>
        </div>
      )}
    </div>
